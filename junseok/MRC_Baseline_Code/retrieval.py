...
class SparseRetrieval_BM25Plus:
    def __init__(self, tokenize_fn,k1=1.5,b=0.75,delta=1, data_path="./data/", context_path="wikipedia_documents.json"):
        self.data_path = data_path
        with open(os.path.join(data_path, context_path), "r") as f:
            wiki = json.load(f)
        ...
        # self.tfidfv.fit(self.context)
        # self.avdl = super(TfidfVectorizer,self.vectorizer).transform(self.context).sum(1).mean()
        # should run get_sparse_embedding() or build_faiss() first.
        self.p_embedding = None
        self.indexer = None
        self.avdl = None
    ...
    def get_relevant_doc_bulk(self, queries, k=1):
        ...
        for i in range(0,len(queries),100):
            subQueries = queries[i:i+100]
            query_vec = self.tfidfv.transform(subQueries)
            print(query_vec.shape)
            assert (
                    np.sum(query_vec) != 0
            ), "오류가 발생했습니다. 이 오류는 보통 query에 vectorizer의 vocab에 없는 단어만 존재하는 경우 발생합니다."
...
class SparseRetrieval_BM25P:
    ...
    def get_relevant_doc(self, query, k=1):
        ...
        self.bm25.get_batch_scores(query, doc_ids)
        return doc_scores, doc_indices
